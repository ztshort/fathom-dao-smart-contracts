...
import "./ERC20StakingRewardsInternals.sol";
import "../staking/vault/interfaces/IVault.sol";
import "../staking/library/BoringMath.sol";

contract ERC20StakingInternals is ERC20StakingStorage, ERC20StakingRewardsInternals {
    ...
    function _initializeStaking(
        ...
        require(_wERC20 != address(0x00), "zero main addrr");
        ...
        weight = _weight;
        vault = _vault;
        lockShareCoef = _lockShareCoef;
        lockPeriodCoef = _lockPeriodCoef;
     ...
     * @param _newLocked The ERC20LockedBalance with zero balances updated through this function
     * @param amount The amount to lock and stake
     */
    function _lock(
        address account,
        ERC20LockedBalance memory _newLocked,
        uint256 amount
    ) internal {
        //@notice: newLock.end is always greater than block.timestamp
        uint256 lockPeriod = _newLocked.end - block.timestamp;
        uint256 lockPeriodWeight = _calculateLockPeriodWeight(amount, lockPeriod);
        if (amount > 0) {
            _newLocked.amountOfERC20 += BoringMath.to128(amount);
        }
    ...
    function _unlock(
        ...
        ERC20LockedBalance storage updateLock = locks[account][lockId - 1];
        uint256 stakeValue = updateLock.amountOfERC20;
     ...
     * @param lockPeriodWeight The amount of vote tokens released
    ...
    function _stake(
        ...
        totalAmountOfStakedERC20 += amount;

        uint256 weightedAmountOfSharesPerStream = _weightedShares(amount, lockPeriodWeight);
     ...
     */
    function _unstake(
        ERC20LockedBalance storage updateLock,
        uint256 stakeValue,
        uint256 lockId,
        address account
    ) internal {
        ...
        totalAmountOfStakedERC20 -= stakeValue;
    ...
    /**
     ...
     @notice The penalty is decreased from the pendings of ERC20 stream
    ...
    function _earlyUnlock(
        ...
        uint256 amount = lock.amountOfERC20;
    ...
    function _removeLockPosition(
        ...
        if (lastLockId != lockId && lastLockId > 1) {
            ERC20LockedBalance storage lastIndexLockedBalance = locks[account][lastLockId - 1];
    ...
    function _withdrawPenalty(address accountTo) internal {
        ...
        IVault(vault).payRewards(accountTo, wERC20, pendingPenalty);
    }
     ...
     * @param amountOfERC20Shares The amount of Shares a user has
     * @param lockPeriodWeight The amount of Vote token for which shares will be calculated
     */
    function _weightedShares(
        uint256 amountOfERC20Shares,
        uint256 lockPeriodWeight
    ) internal view returns (uint256) {
        ///@notice Shares accomodate vote the amount of  ERC20Shares and vote Tokens to be released
        ...
        uint256 shares = amountOfERC20Shares + (lockShareCoef * lockPeriodWeight) / 1000;
        return shares;
    }
     ...
     * @notice
     */
    function _calculateLockPeriodWeight(uint256 amount, uint256 lockingPeriod) internal view
             returns (uint256 lockPeriodWeight) {
        //voteWeight = 365 * 24 * 60 * 60;
        lockPeriodWeight = (amount * lockingPeriod * POINT_MULTIPLIER) / lockPeriodCoef / POINT_MULTIPLIER;
        return lockPeriodWeight;
    }

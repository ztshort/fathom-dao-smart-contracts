...
interface IERC20StakingGetter {
    function getLatestRewardsPerShare(uint256 streamId) external view returns (uint256);

    function getLockInfo(address account, uint256 lockId) external view returns (ERC20LockedBalance memory);
    ...
    function getAllLocks(address account)  external view  returns (ERC20LockedBalance[] memory);

...
import "./interfaces/IERC20StakingEvents.sol";

contract ERC20StakingRewardsInternals is ERC20StakingStorage, IERC20StakingEvents {
    ...
    function _moveRewardsToPending(
        ...
        require(streams[streamId].status == ERC20StreamStatus.ACTIVE, "inactive or proposed");
    ...
    function _moveAllStreamRewardsToPending(address account, uint256 lockId) internal {
        ...
        for (uint256 i = 0; i < streamsLength; i++) {
            if (streams[i].status == ERC20StreamStatus.ACTIVE) _moveRewardsToPending(account, i, lockId);
        }
    }
    ...
    function _moveAllLockPositionRewardsToPending(address account, uint256 streamId) internal {
        ...
        require(streams[streamId].status == ERC20StreamStatus.ACTIVE, "inactive or proposed");
        ERC20LockedBalance[] memory locksOfAccount = locks[account];
    ...
    function _before() internal {
        ...
        uint256 streamsLength = streams.length;
        if (totalAmountOfStakedERC20 != 0 && streamsLength != 0) {
            for (uint256 i = 0; i < streamsLength; i++) {
                if (streams[i].status == ERC20StreamStatus.ACTIVE) {
    ...
    function _rewardsSchedule(
        ...
        ERC20Schedule storage schedule = streams[streamId].schedule;
        uint256 startIndex;
        uint256 endIndex;
